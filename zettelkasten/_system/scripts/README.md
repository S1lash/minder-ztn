...
| `migrate_legacy_frontmatter.py` | One-shot migration helper for legacy frontmatter shapes (e.g. snake_case domain values, `audience` → `audience_tags`). Idempotent — re-running on already-migrated files is a no-op. | frontmatter of in-scope `.md` files |
