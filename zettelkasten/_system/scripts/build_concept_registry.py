...
Output: `_system/registries/CONCEPTS.md` with `## Top by mentions`
(loaded by `/ztn:process` Step 3.5 subagent) and `## Tail` sections,
split at `--top-threshold` (default 100).
...
Usage:
    python3 build_concept_registry.py [--top-threshold N] [--dry-run]
"""
...
def render_registry(agg: dict[str, ConceptAgg],
    ...
    top = [c for c in sorted_concepts if c.mentions >= top_threshold]
    tail = [c for c in sorted_concepts if c.mentions < top_threshold]
    ...
    top_section = (
        "## Top by mentions (loaded by /ztn:process Step 3.5 subagent)\n\n"
        f"{table_header}"
        + "\n".join(_format_row(c) for c in top)
        + ("\n" if top else "_(none — corpus has no concept above threshold yet)_\n")
        + "\n"
    )

    tail_section = (
        "## Tail (full registry — not loaded into Step 3.5 by default)\n\n"
        f"{table_header}"
        + "\n".join(_format_row(c) for c in tail)
        + ("\n" if tail else "_(none)_\n")
    )

    return header + top_section + "\n" + tail_section


def build(base: Path, top_threshold: int) -> tuple[dict[str, ConceptAgg], dict]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--top-threshold", type=int, default=DEFAULT_TOP_THRESHOLD)
    ...
    agg, stats = build(base, args.top_threshold)
    rendered = render_registry(agg, args.top_threshold)
