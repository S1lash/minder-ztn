...
**Opportunity lens (DRY handoff).** Возможности, серендипность, судьба
прошлых веток и развилки — **собственность линзы `opportunity`** (SoT
opportunity-логики). Читай её последний вывод
(`_system/agent-lens/opportunity/`) как один из входов — так же, как
читаешь `biometric-life-synthesis`. **Не форкасти standalone-возможности
независимо** (это была бы двойная работа): поднимай opportunity-материал
ТОЛЬКО когда он конвергирует с другим сигналом недели (opportunity +
energy + stated-vs-lived → одна картина). Если у недели нет такой
конвергенции — не тащи возможности в digest, они уже раскрыты в своей
линзе. Не-конвергенция (не тащить): `opportunity` называет человека/дверь,
а ничего другого в сигналах недели её не касается → оставь её линзе
`opportunity`, это не тема weekly-insights.
...
звонок с Артёмом 27.04», «хаб про office-move», «твоя
...
- **Forecast / failure-mode по reference class** — владелец был в
  ...
  предсказания, ref-class. **NB (opportunity DRY):** это про
  повторяющийся failure-mode/траекторию, НЕ про standalone-возможности
  или серендипность — те принадлежат линзе `opportunity` (handoff в
  «Что читать»). Не форкасти новую дверь под видом Forecast/Drift.
