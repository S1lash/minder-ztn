...
| `role-guard-evaded` | `/ztn:roles` | The run changed something the guard cannot account for: git's own configuration (a remote's URL, `core.hooksPath`, an index flag, `.git/config`) differed between the snapshots, an ignore change made something invisible that was neither ignored nor visible before, or the working tree kept changing after the role finished. Both mean the role acted on the check itself rather than within it — the guard's whole model is that the repository is what `git status` prints, and these move that ground | Surfaced on the FIRST occurrence, never deferred to a repeat: the run is `error`, the tick stops dispatching further roles (every later check and push would stand on the changed ground), and nothing is repaired — reverting would mean writing inside `.git/`. Names the exact field with its before and after, or the paths that moved after the run. Suggested action `inspect-git-state` |
| `role-ignore-changed` | `/ztn:roles` | Git's ignore rules differed between the snapshots, but nothing became invisible: every newly-ignored path was either ignored already or visible on disk when the role started, so no payload could be hiding behind the change. The ordinary author is whatever hosts the tick maintaining its own runtime entries in `.git/info/exclude`, a file the guard may not write to | The run is `ok` and the tick keeps dispatching — an abort that fires on an ordinary night stops being read and takes the roles queued behind it down with it. Names the ignore files that differ and both digests, and states plainly that nothing became invisible. Carries NO credential-compromise warning — that belongs to an evasion. Self-quieting: once the host's entries are in place the file stops changing, so this surfaces once rather than nightly. Suggested action `acknowledge` |
...
| `fix-role` | `{role-id}` | `action: "edited | paused", note: "..."` — a `role-repeated-error` item; owner repairs the assignment via `/ztn:role:edit` or pauses it |
