...
- `tick-telemetry.jsonl` — written ONLY by `scripts/scheduler/record_telemetry.py`, invoked at Step 4.9 of each scheduler tick (append-only, one line per tick, measured from the run's own transcript). Ticks only: a manual skill run writes nothing here, by design. The writer always exits 0, so `/ztn:lint` Scan A.13 is the only thing that can notice it stopped
