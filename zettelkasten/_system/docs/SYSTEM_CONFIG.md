...
`/ztn:resolve-clarifications` acquires `.resolve.lock` for both interactive and `--auto-mode` runs and reads the four pipeline locks (process / maintain / lint / agent-lens) at start. Lock check is fully symmetric — auto-mode aborts on any pipeline lock just like interactive mode does. The three nightly skills (`lint` 03:00, `agent-lens` 03:30, `resolve --auto-mode` 04:00) run as separate scheduler ticks specifically so each gets a fresh scheduler-agent context for its LLM judgements; the 30-minute spacing also keeps lock contention essentially impossible (each tick commits and pushes within ~5-15 min before the next pulls).
