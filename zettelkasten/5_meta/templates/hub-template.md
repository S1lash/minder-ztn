...
chronological_map_mode: derived  # derived (project hubs, auto-filled by render_hub_maps.py) | curated (trajectory / domain hubs, hand-curated). Absent → curated
# `derived` only: record-ids the map must skip, plus a parallel list of
# reasons that MUST be the same length.
excluded_from_map: []
...
2. **Хронологическая карта** — таймлайн всех связанных заметок с датами. При
   `chronological_map_mode: curated` секцию ведёт владелец. При `derived`
   её владеет `_system/scripts/render_hub_maps.py`: на первом же прогоне
   `/ztn:maintain` он заменяет секцию блоком между маркерами
   `<!-- AUTO-GENERATED by render_hub_maps.py … -->`, и всё, что было
   вписано внутрь руками, пропадает.
