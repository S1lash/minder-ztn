...
### ADR-028: Что ролевая подсистема потеряла при перестройке

**Контекст:** Первая ролевая подсистема просила стоящую задачу уместиться в
фиксированный словарь — закрытый набор видов памяти, закрытый набор форм
действия, путь чтения, который не умел POST и потому действовал на доске,
которую не мог прочитать. Её сняли целиком и пересобрали на форме, где движок
ограничивает только **где** роль пишет и **до чего** дотягивается. Это решение
верное и оно записано.

Не записано было другое: вместе со словарём ушли пять гарантий, которые
исполнялись **кодом**, а в новой форме держатся на тексте промпта или не
держатся ничем. Пробел не в том, что их убрали, — а в том, что их убрали молча,
и следующий человек, читая «упрощение», не узнает, чем за него заплачено.

**Что действовало раньше и чем стало:**

1. **Белый список внешних адресатов.** Был реестр разрешённых целей; роль не
   могла обратиться никуда помимо него. Сейчас реестра нет — роль получает
   оболочку и расшифрованный файл с ключами, то есть достижимо всё, до чего
   дотянется обычный сетевой вызов.
2. **Аудит каждого внешнего вызова.** Была строка на вызов (хэш, сводка) плюс
   флаг «во входе есть внешние данные» — метка для защиты от подмены
   инструкций. Сейчас пишется одна строка на **запуск**, не на вызов.
3. **Бюджет исходящих записей.** Был накопительный лимит за период; модуль сам
   называл его предпосылкой, а не поздней добавкой. Сейчас ограничение только
   по времени выполнения.
4. **Гейт срабатывания, вычисляемый снаружи тела роли.** Решение «сегодня
   нечего делать» принимал runner до запуска тела и записывал его; тело не
   могло объявить это само, потому что такое самоописание непроверяемо. Сейчас
   тело сообщает это о себе.
5. **Фильтрация чувствительного на уровне чтения.** Запрос отбрасывал чужие
   sensitive-сущности до того, как они попадали в контекст. Сейчас это
   предложение в преамбуле.

**Следствие:** гарантии 1–3 и 5 — про внешние действия и приватность, и их
отсутствие имеет значение ровно в тот момент, когда роль получает право писать
наружу. Гарантия 4 — про честность отчёта, и она уже нарушена по форме.
Восстанавливать сейчас ничего не требуется: новая форма пока пишет
преимущественно в репозиторий, где границы держит проверка диффа после запуска.
Запись существует, чтобы решение расширить роли наружу принималось с открытыми
глазами, а не как «просто включим ещё один инструмент».

**Альтернатива, которую отвергли:** восстановить словарь. Он и был причиной
снятия — задача не влезала в него, а не он в задачу.
