...
| `zettelkasten/_system/roles/{id}/` (`config.yml`, `hooks/`, `brief.md?`, `parts/*.json`, `state.md`, `decisions.jsonl`, `triggers.json`, `budget.json`, `pending_acts.json`) | `/ztn:role:add` (create) + `/ztn:role:edit` (change / lifecycle) own identity (config + hooks + optional brief, owner-sovereign, validate-before-write, NEITHER seeds part state) + `/ztn:roles` via `roles_persist.py` (a role is a composite of parts; `parts/{id}.json` / `state.md` AUTO sub-zones / decisions.jsonl — sole deterministic writer; the tick body NEVER writes) + `/ztn:roles` runner-owned cross-tick state (`triggers.json` via `roles_triggers.py` — trigger-gate watermark + skip-streak; `budget.json` via `roles_budget.py` — cumulative act/inbox ceiling; `pending_acts.json` via `roles_persist.py` — outward acts staged in Phase 1, executed + cleared by `--approve-acts` in Phase 2, §6.5) |
| `zettelkasten/_system/state/{roles-tool-audit.jsonl, secrets.enc.json}` | `roles-tool-audit.jsonl` — `/ztn:roles` via `roles_tool_stage.py` (append-only tool-call audit: hash+summary only, never the raw return — INV-10). `secrets.enc.json` — `/ztn:role:add` concierge via `roles_secrets.py` (encrypted credential blob; master key via `ZTN_SECRET_MASTER_KEY`, never git) |
...
integrations/claude-code/skills/
  ...
  ztn-role-ask/         ztn-role-edit/       ztn-role-list/
```
...
| `zettelkasten/_system/registries/TOOLS.md` (+ `TOOLS.template.md`) | Roles-tools whitelist — read + act tool specs (data), the `Act Config` schema, the act-tools-run doc. Owner data (`exclude:`); the spec + a baseline ship via the strip-seed template. Act contract SoT — `platform/roles/CONTRACT.md` |
| `integrations/claude-code/skills/ztn-role-add/SKILL.md` | Role/tool creation concierge — its expertise is **hardcoded NARRATIVE prose that drifts silently**. When the role- or tool-engine gains an adapter kind / mandate field / grounding mode / part archetype / act shape, update its taxonomy prose + owner disclosures + verify-at-creation walk in the SAME change. Maintainer compatibility checklist lives in its own `## Future-proof notes (for engine maintainers)` section — open it before touching the tool-engine. Contract SoT — `platform/roles/CONTRACT.md` |
| `integrations/claude-code/skills/ztn-roles/SKILL.md` | Role tick runner + `--approve-acts`/`--approve-coldstart` handling. When the tool-engine gains an act shape / mandate field / trigger kind / budget rule / clarification type, update the tick pipeline + the act-approval flow in lockstep. Act contract SoT — `platform/roles/CONTRACT.md` §6.2/§6.5 |
...
- `/ztn:roles --all-due` — pre-sync → role ticks → finalize (daily, 06:00)
