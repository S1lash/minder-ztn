...
`CURRENT_CONTEXT.md: pending`. After Step 7 completes successfully:
...
3. Append a line immediately below noting the Step 7.5 pattern-detect
...
Earlier batches in the run retain their `pending` note — readable as
«covered by the last batch's regen», consistent with invariant that regen
runs once per run, not once per batch.

If Step 7 was skipped (e.g. `--dry-run`), leave `pending` in place and
also skip the pattern-detect confirmation — no patch needed.
...
6. **CURRENT_CONTEXT regenerated when batches were processed** — if at least
   one batch reached Step 6.5, Step 7 runs once post-loop. Exceptions:
   `--dry-run` (no regen), empty unprocessed set (never reaches post-loop —
   Early Exit handles it).
...
8. **Empty batch safe** — a batch with 0 records, 0 notes, 0 tasks still
   ...
   log_maintenance.md entry written with zeroed counters. CURRENT_CONTEXT regen
   still runs post-loop (provides fresh timestamp even if state didn't change).
