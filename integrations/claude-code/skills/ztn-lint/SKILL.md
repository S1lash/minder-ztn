...
underlying knowledge layer. INDEX is regenerated by `/ztn:maintain`
Step 7.6 — staleness signals that maintain has not been running, or
that a hand-edit replaced the auto-zone.

Pipeline:
1. Read `_system/views/INDEX.md` frontmatter `generated:` timestamp.
2. Compute `latest_modified` across knowledge layer (`1_projects/`,
   `2_areas/`, `3_resources/` excluding registries) + hubs
   (`5_meta/mocs/`) — max of frontmatter `modified:` (or `created:`
   fallback).
...
4. If `INDEX.generated < latest_modified - 7 days` → `weak` floor,
   ...
   {date}; newest knowledge note is {date} ({delta} days drift)».
   Surfaced tier (never auto-apply — INDEX regen happens via maintain
   pipeline, not lint).
